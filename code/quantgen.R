...
quantgen_forecaster = function(
  ...
  #for (i in 1:length(ahead)) {
    ...
    # sets of features on n training dates
...
# the first list elem is the prediction df
result = lapply(results_list, function(x) x[[1]])
# the second elem contains input params and the fitted
# model params from gurobi
if (!is.null(debug)) {
  saveRDS(list(             
   ahead = ahead,
